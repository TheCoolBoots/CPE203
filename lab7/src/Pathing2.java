...
public class Pathing2 extends PApplet {
	...
	private void draw_path() {
		if (drawPath) {
			for (Point p : path) {
				fill(128, 0, 0);
				rect(p.x * TILE_SIZE + TILE_SIZE * 3 / 8, p.y * TILE_SIZE + TILE_SIZE * 3 / 8, TILE_SIZE / 4,
						TILE_SIZE / 4);
			}
		}
	}
	 ...
	 */
	private boolean moveOnce(Point pt) {
		...
		if (withinBounds(pt, grid) && grid[pt.y][pt.x] != GridValues.OBSTACLE
				&& grid[pt.y][pt.x] != GridValues.SEARCHED) {

			//check if cell is goal
			if (grid[pt.y][pt.x] == GridValues.GOAL) {
				return true;
			}

			//keep searching starting with the point directly to the right
			else {
				grid[pt.y][pt.x] = GridValues.SEARCHED;
				//System.out.println(pt);
				rightOk = moveOnce(pt.getTop());  //If found through right path, don't look through other paths
				if(!rightOk)
					botOk = moveOnce(pt.getBot());
				if(!botOk && !rightOk)
					leftOk = moveOnce(pt.getLeft());
				if(!leftOk && !botOk && !rightOk)
					topOk = moveOnce(pt.getRight());
				
				if (rightOk || botOk || leftOk || topOk) {  //if this point has found a way to get to the target, add to path
					path.add(pt);
					return true;
				}
